...
bool visited[101][101];	//방문을 확인할 배열 ->0으로 초기화 됨
int n, m;	//가로,세로의 값을 입력받은 변수
int xarr[4] = { 1,0,-1,0 };	//오른쪽,왼쪽을 확인할 배열
int yarr[4] = { 0,1,0,-1 }; //위,아래 
...
int bfs(int x, int y)	//bfs 함수
{
	...
	char temp = map[x][y];	//temp에 해당위치에 병사값을 저장 -> 처음 위치인 부모노드 탐색
		...
		for (int i = 0; i < 4; i++)	//4가지방향을 탐색하기 때문에 4번 반복하는 반복문
		{
			...
			if (0 <= na && na < m && 0 <= nb && nb < n)	//map을 벗어나지않는 경우
			{
				if (visited[na][nb] == false && map[na][nb] == temp)	//방문하지않은 위치면서 매개변수로 받은 병사값과 같을경우
				{
					...
					count++;	//count에 1을더한다
					q.push(make_pair(na, nb));	//queue에 push ->자식노드를 탐색
				}
			}
